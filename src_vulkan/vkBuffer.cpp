...
namespace vk 
{
		...
		size_t size, void* data )
	{
		...
		vkGetBufferMemoryRequirements(c_device, m_handle, &memoryRequirements);

		VkMemoryAllocateInfo vmai = vk::init::MemoryAllocateInfo();
		vmai.allocationSize = memoryRequirements.size;

		VkPhysicalDeviceMemoryProperties	vpdmp;
		vkGetPhysicalDeviceMemoryProperties(devicePtr->GetGPU(), &vpdmp);

		vmai.memoryTypeIndex = devicePtr->GetMemoryType(memoryRequirements.memoryTypeBits, flags);
