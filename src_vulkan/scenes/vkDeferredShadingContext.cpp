...
namespace vk {
	...
	void DeferredContext::InitializeGraphicsPipelineLayout()
	{
		...
		//COMPOSITION PASS LAYOUT
		{
			if (pipelineLayouts[dePipeline::COMPOSITION] == VK_NULL_HANDLE)
			{
				...
				std::array<VkDescriptorSetLayout, 2> composition_layouts = {
					...
					compositionSamplerDescriptor.GetLayout(), uniformBindingDescriptors[dePipeline::COMPOSITION].GetLayout()
				};
		...
		//SKYBOX LAYOUT
		{
			if (pipelineLayouts[dePipeline::SKY] == VK_NULL_HANDLE)
			{
				std::array<VkDescriptorSetLayout, 2> layouts =
				{
					...
					uniformBindingDescriptors[dePipeline::MRT].GetLayout(), skyboxSamplerDescriptor.GetLayout()
				};
		...
		//SWAPCHAIN QUAD LAYOUT
		{
			if (pipelineLayouts[dePipeline::SWAPCHAIN] == VK_NULL_HANDLE)
			{
				std::array<VkDescriptorSetLayout, 1> layouts = {
					//set 0: scene sampler
					swapChainSamplerDescriptor.GetLayout()
				};
	...
	void DeferredContext::ResizeWindow()
	{
		...
		/*compositionSamplerDescriptor.Destroy();
		swapChainSamplerDescriptor.Destroy();

		InitializeCompositionSamplerDescriptor();
		InitializeSwapChainDescriptor();*/
	}

	void DeferredContext::Render( Scene& scene )
	{
		if (PrepareFrame())
		{ 
			UpdateScreenUniforms();
			UpdateLights();
			RecordCommandBuffers(scene);
			SubmitFrame();
		}
	}
