...
#include "scenes/vkDeferredShadingContext.h"
...
void Application::init() 
{
	...
	m_renderer = std::make_unique<vk::DeferredContext>();
	m_renderer->Init();

	const auto& graphicsContextInfo = m_renderer->GetGraphicsContextInfo();

	m_resourceManager.Init(graphicsContextInfo.devicePtr, 2);
...
void Application::loop()
{
	if (m_renderer != nullptr)
	{
		...
		while (exitApplication == false)
		{
			...
			Controller::MoveCamera(m_renderer->GetCamera(), static_cast<float>(realFrameTime));
			...
			m_sceneManager.Update(static_cast<float>(realFrameTime));
