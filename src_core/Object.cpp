...
#define GLTF_OBJECT_PATH "assets/gltf/"
#define OBJ_PATH "assets/obj/"

Object::Object( const ObjectCreateInfo& objectCI, ResourceManager& resourceManager )
{
...
void Object::InitPhysics( PhysicsSystem& physicsWorld )
{
    ...
    m_physicsComponent.rigidBody = physicsWorld.AddRigidBody(transform);
    ...
    if (m_physicsComponent.colliderType == PhysicsComponent::ColliderType::CUBE) 
    {
        ...
        m_physicsComponent.shape = physicsWorld.CreateBoxShape({ std::abs(worldHalfExtent.x), std::abs(worldHalfExtent.y), std::abs(worldHalfExtent.z) });
    }
...
void Object::Update( const float& interpFactor )
{
        ...
        m_physicsComponent.bodyType != BodyType::STATIC)
    {
        ...
        m_physicsComponent.currTransform =
            Transform::interpolateTransforms(m_physicsComponent.prevTransform, uninterpolatedTransform, interpFactor);
